  ...
  return (
    <section id="experience" ref={ref} className="container relative py-16 sm:py-24 lg:py-32">
      ...
      <div className="relative mx-auto max-w-4xl">
        ...
        <motion.div className="absolute left-4 top-0 w-px bg-primary sm:left-1/2" style={{ height }} />
          ...
          {experience.map((item, index) => (
            <Reveal key={`${item.company}-${item.period}`}>
              <article className={`relative grid gap-6 sm:grid-cols-2 ${index % 2 === 0 ? "" : "sm:[&>div]:col-start-2"}`}>
                <span className="absolute left-[0.55rem] top-6 size-4 rounded-full border border-primary bg-background sm:left-[calc(50%-0.45rem)]" />
